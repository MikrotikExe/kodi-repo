...
try:
    ...
    _DATA_DIR = xbmcvfs.translatePath('special://profile/addon_data/%s/' % _ADDON_ID)

    def _log(level, msg):
        try:
            xbmc.log('[%s] imdb_lookup: %s' % (_ADDON_ID, msg), level)
        except Exception:
            pass

    # Whether the IMDb lookup feature is enabled is determined by the
    # existence of an empty marker file in the addon's data directory:
    #   special://profile/addon_data/plugin.video.tvheadend/imdb_lookup_enabled
    #
    # The reason we do not call _ADDON.getSettingString('online_metadata_lookup')
    # for every entry is that Kodi 21 emits "Invalid setting type" to the
    # error log on every read of a setting that has never been written to
    # the user storage. After an addon upgrade, new settings from
    # settings.xml are NOT auto-registered in Kodi's settings DB — they get
    # the default value only the first time the user opens the Settings
    # dialog. We need to know the value BEFORE that happens, hence the
    # marker file (managed by handler_settings() in default.py).
    # Cache whether we've already logged the enabled/disabled state once
    # per Kodi session — for diagnostics in the log.
    _SETTING_STATE_LOGGED = False

    def _setting_enabled():
        global _SETTING_STATE_LOGGED
        try:
            flag = os.path.join(_DATA_DIR, 'imdb_lookup_enabled')
            enabled = os.path.isfile(flag)
        except Exception:
            enabled = False
        if not _SETTING_STATE_LOGGED:
            _SETTING_STATE_LOGGED = True
            _log(1, 'feature %s (flag file: %s)' % (
                'ENABLED' if enabled else 'disabled',
                os.path.join(_DATA_DIR, 'imdb_lookup_enabled')))
        return enabled

    def _flag_path():
        return os.path.join(_DATA_DIR, 'imdb_lookup_enabled')
except ImportError:
    # Standalone test mode (no Kodi)
